...
from aiogram.types import (
    ...
    LabeledPrice, PreCheckoutQuery, SuccessfulPayment, InlineQueryResultArticle,
...
class AdminStates(StatesGroup):
    ...
    waiting_withdraw_cooldown = State()
...
async def get_user(user_id: int) -> dict:
    ...
    if user.get("banned_until"):
        ban_until = datetime.fromisoformat(user["banned_until"])
        if datetime.now() < ban_until:
            return user
        else:
            await update_user(user_id, banned_until=None)
...
# Система ограничений вывода
async def check_withdrawal_limit(user_id: int, amount: int) -> Tuple[bool, str]:
    user = await get_user(user_id)
    settings = await load_json(SETTINGS_FILE, {})
    ...
    # Проверка дневного лимита выводов
    max_per_day = settings.get("max_withdraw_per_day", 50000)
    if user.get("daily_withdrawals", 0) + amount > max_per_day:
        return False, f"❌ Дневной лимит выводов: {max_per_day} ⭐. Сегодня вы уже вывели {user.get('daily_withdrawals', 0)} ⭐"
    
    # Проверка кулдауна
    cooldown_minutes = settings.get("withdraw_cooldown_minutes", 60)
    last_withdraw = user.get("last_withdraw")
    if last_withdraw:
        last_time = datetime.fromisoformat(last_withdraw)
        ...
        if time_passed < cooldown_minutes:
            ...
            return False, f"❌ Следующий вывод будет доступен через {remaining} минут"
...
async def update_withdrawal_stats(user_id: int, amount: int):
    ...
    await update_user(user_id, 
                     daily_withdrawals=user.get("daily_withdrawals", 0) + amount,
                     last_withdraw=datetime.now().isoformat())

# Система ограничений игры
async def check_user_limits(user_id: int, bet: int, game: str) -> Tuple[bool, str]:
    ...
    daily_loss_limit = settings.get("max_daily_loss", 50000)
    if user.get("daily_loss", 0) + bet > daily_loss_limit:
        return False, f"❌ Дневной лимит потерь: {daily_loss_limit} ⭐. Сегодня вы уже проиграли {user.get('daily_loss', 0)} ⭐"
    
    # Проверка дневного лимита выигрыша
    daily_win_limit = settings.get("max_daily_win", 100000)
    if user.get("daily_win", 0) >= daily_win_limit:
        return False, f"❌ Дневной лимит выигрыша: {daily_win_limit} ⭐. Вы уже достигли лимита!"
    
    # Проверка максимальных последовательных проигрышей
    max_losses = settings.get("max_consecutive_losses", 10)
    if user.get("consecutive_losses", 0) >= max_losses:
...
async def update_user_limits(user_id: int, amount: int, is_win: bool = False, is_loss: bool = False):
    ...
    if is_loss:
        await update_user(user_id, 
                         daily_loss=user.get("daily_loss", 0) + amount, 
                         consecutive_losses=user.get("consecutive_losses", 0) + 1)
    elif is_win:
        await update_user(user_id, 
                         daily_win=user.get("daily_win", 0) + amount, 
                         consecutive_losses=0)
...
class CrashGame:
    ...
    async def update_multiplier(self) -> float:
        crash_probability = 1 / (self.current_multiplier * 6)
...
class MinesGame:
    ...
    def get_multiplier(self) -> float:
        ...
        if opened_safe == 0:
            return 1.0
        multiplier = 1.0 + (opened_safe / safe_cells) * 4.5
...
async def casino_rocket(user_id: int, bet: int) -> Tuple[bool, int, str]:
    ...
    if multiplier >= cash_out_point:
        winnings = int(bet * cash_out_point)
        await add_stars(user_id, winnings, f"Выигрыш в Ракете (казино)")
        user = await get_user(user_id)
        await update_user(user_id, casino_wins=user["casino_wins"] + 1)
        await update_tournament_points(user_id, winnings)
        await update_user_limits(user_id, winnings, is_win=True)
...
async def casino_number_guess(user_id: int, bet: int, choice: int) -> Tuple[bool, int, str]:
    ...
    if win:
        winnings = int(bet * 10)
        await add_stars(user_id, winnings, f"Выигрыш в Угадай число (казино)")
        user = await get_user(user_id)
        await update_user(user_id, casino_wins=user["casino_wins"] + 1)
        await update_tournament_points(user_id, winnings)
        await update_user_limits(user_id, winnings, is_win=True)
...
async def inline_check(inline_query: InlineQuery):
    ...
    if query.startswith("check_"):
        ...
        for check in checks["checks"]:
            if check["code"] == code and not check["used"]:
                result = InlineQueryResultArticle(
                    ...
                    description=f"Активировать чек и получить {check['amount']} звезд",
                    input_message_content=InputTextMessageContent(
                        message_text=f"🎫 *Чек активирован!*\n\n💰 Получено: {check['amount']} ⭐\n👤 От: {check['creator']}",
                        parse_mode="Markdown"
                    ),
        ...
        result = InlineQueryResultArticle(
            id="not_found",
            title="❌ Чек не найден",
            description="Чек с таким кодом не существует или уже использован",
            input_message_content=InputTextMessageContent(
                message_text="❌ Чек не найден или уже использован!",
                parse_mode="Markdown"
            )
        )
        await inline_query.answer([result], cache_time=0)
    ...
    results = [
        InlineQueryResultArticle(
            id="info",
            title="ℹ️ Информация",
            description="Как использовать чеки",
            input_message_content=InputTextMessageContent(
                message_text="📦 *Как использовать чеки*\n\n1. Получите код чека от друга\n2. Отправьте боту /use_check <код>\n3. Или используйте инлайн режим: @bot check_<код>\n\nЧеки можно создавать в меню 'Чек система'",
                parse_mode="Markdown"
            )
        )
    ]
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if user.get("banned_until"):
        ban_until = datetime.fromisoformat(user["banned_until"])
        if datetime.now() < ban_until:
            ban_text = f"\n\n⚠️ *ВЫ ЗАБАНЕНЫ ДО {ban_until.strftime('%d.%m.%Y %H:%M')}* ⚠️"
    
    text = (f"✨ Добро пожаловать, {message.from_user.full_name}!{ban_text}\n\n"
            ...
            f"└ Дневной лимит выводов: {settings.get('max_withdraw_per_day', 50000)} ⭐\n"
            f"└ Кулдаун выводов: {settings.get('withdraw_cooldown_minutes', 60)} минут\n"
...
async def games_menu(callback: CallbackQuery):
    ...
    for game_id, game in GAMES.items():
        builder.row(InlineKeyboardButton(text=game['name'], callback_data=f"game_{game_id}"))
...
async def show_mines_field(message: Message, state: FSMContext, mines_game: MinesGame):
    ...
    for i in range(5):
        for j in range(5):
            ...
            if cell in opened:
                ...
                text = "❓"
            builder.add(InlineKeyboardButton(text=text, callback_data=f"mines_cell_{cell}"))
        builder.adjust(5)
    
    builder.row(InlineKeyboardButton(text="💰 Забрать выигрыш", callback_data="mines_cashout"))
    builder.row(InlineKeyboardButton(text="🔙 Выход", callback_data="games_menu"))
    ...
    mines_game.message_id = msg.message_id
    await state.update_data(mines_game=mines_game)
...
async def casino_menu(callback: CallbackQuery):
    ...
    for game_id, game in CASINO_GAMES.items():
        builder.row(InlineKeyboardButton(text=game['name'], callback_data=f"casino_{game_id}"))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_main"))
...
async def casino_bet(message: Message, state: FSMContext):
    ...
    if game == "slots":
        ...
        win, winnings, result_text = await casino_rocket(message.from_user.id, bet)
        await message.answer(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🚀 Ещё раз", callback_data="casino_rocket"),
             InlineKeyboardButton(text="🔙 В меню", callback_data="casino_menu")]
        ]))
        await state.clear()
...
@dp.message(CasinoStates.waiting_number_guess)
async def casino_number_guess(message: Message, state: FSMContext):
    try:
        choice = int(message.text)
        if choice < 1 or choice > 10:
            await message.answer("❌ Введите число от 1 до 10!")
    ...
    win, winnings, result_text = await casino_number_guess(message.from_user.id, bet, choice)
    await message.answer(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔢 Ещё раз", callback_data="casino_number_guess"),
         InlineKeyboardButton(text="🔙 В меню", callback_data="casino_menu")]
    ]))
    await state.clear()
...
async def withdraw_menu(callback: CallbackQuery):
    ...
    text = (f"💰 *Вывод звезд*\n\n⭐ Баланс: {user['stars']}\n"
            ...
            f"📊 Дневной лимит: {settings.get('max_withdraw_per_day', 50000)} ⭐\n"
            f"⏰ Кулдаун: {settings.get('withdraw_cooldown_minutes', 60)} минут\n\n"
...
async def withdraw_stars(message: Message):
    ...
    limit_ok, limit_msg = await check_withdrawal_limit(message.from_user.id, amount)
    if not limit_ok:
        await message.answer(limit_msg)
        return
    
    await remove_stars(message.from_user.id, amount, f"Вывод звезд")
    await update_withdrawal_stats(message.from_user.id, amount)
...
async def check_system_menu(callback: CallbackQuery):
    ...
    if user.get("check_system_unlocked", False):
        text = (f"📦 *Система чеков*\n\n"
                ...
                f"🎫 Активировать чек:\n"
                f"└ /use_check <код>\n"
                f"└ @{BOT_USERNAME} check_<код>\n\n"
                f"💡 Инлайн режим: введите @{BOT_USERNAME} check_<код> в любом чате!")
        ...
        text = (f"📦 *Система чеков*\n\n"
                ...
                f"• Использовать инлайн режим для активации чеков")
...
async def create_check_amount(message: Message, state: FSMContext):
    ...
    if success:
        ...
        await message.answer(
            ...
            f"💡 Инлайн активация: @{BOT_USERNAME} check_{code}",
            parse_mode="Markdown",
            reply_markup=keyboard
        )
    else:
...
async def show_stats(callback: CallbackQuery):
    ...
    text = (f"📊 *Ваша статистика*\n\n⭐ Баланс: {user['stars']}\n💰 Заработано: {user['total_earned']}\n"
            ...
            f"📊 Дневные выводы: {user.get('daily_withdrawals', 0)} ⭐\n"
...
async def show_help(callback: CallbackQuery):
    ...
    text = (f"❓ *Помощь*\n\n"
            ...
            f"• Активация чека: по кнопке, /use_check <код> или @{BOT_USERNAME} check_<код>\n\n"
...
# Админ панель (полная реализация - сокращена для краткости, но все функции есть)
...
async def admin_panel(callback: CallbackQuery):
    ...
    await callback.message.edit_text("⚙️ *Админ панель*", parse_mode="Markdown", reply_markup=builder.as_markup())
    await callback.answer()
...
async def admin_users_menu(callback: CallbackQuery, state: FSMContext):
    ...
    builder.row(InlineKeyboardButton(text="📊 Топ пользователей", callback_data="admin_top_users"))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="admin_panel"))
    
    await callback.message.edit_text("👥 *Управление пользователями*", parse_mode="Markdown", reply_markup=builder.as_markup())
    await callback.answer()
...
async def admin_show_user(message: Message, state: FSMContext):
    ...
    try:
        ...
        text = (f"👤 *Пользователь {user_id}*\n\n⭐ Баланс: {user['stars']}\n💰 Заработано: {user['total_earned']}\n"
                ...
                f"🚫 Бан: {'Да' if user.get('banned_until') else 'Нет'}\n"
...
async def admin_ban_user(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🚫 Введите длительность бана в часах (0 для снятия бана):", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Отмена", callback_data="admin_users")]
    ]))
    await callback.answer()
...
async def admin_process_ban(message: Message, state: FSMContext):
    try:
        ...
        if hours > 0:
            ...
            await update_user(user_id, banned_until=ban_until.isoformat())
            await message.answer(f"✅ Пользователь {user_id} забанен до {ban_until.strftime('%d.%m.%Y %H:%M')}")
            await log_admin_action(message.from_user.id, "ban_user", str(user_id), f"Hours: {hours}")
        else:
            await update_user(user_id, banned_until=None)
            await message.answer(f"✅ Бан снят с пользователя {user_id}")
...
# Остальные админ функции (сокращены, но полные в предыдущих версиях)
...
async def admin_tasks_menu(callback: CallbackQuery, state: FSMContext):
    ...
    builder.row(InlineKeyboardButton(text="📋 Список заданий", callback_data="admin_list_tasks"))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="admin_panel"))
    
    await callback.message.edit_text("📋 *Управление заданиями*", parse_mode="Markdown", reply_markup=builder.as_markup())
    await callback.answer()
...
async def admin_promo_menu(callback: CallbackQuery, state: FSMContext):
    ...
    builder.row(InlineKeyboardButton(text="📋 Список промокодов", callback_data="admin_list_promo"))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="admin_panel"))
    
    await callback.message.edit_text("🎫 *Управление промокодами*", parse_mode="Markdown", reply_markup=builder.as_markup())
    await callback.answer()
...
async def admin_tournaments_menu(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="➕ Создать турнир", callback_data="admin_create_tournament"))
    if active:
        builder.row(InlineKeyboardButton(text="🏆 Текущий турнир", callback_data="admin_current_tournament"))
    builder.row(InlineKeyboardButton(text="📜 История турниров", callback_data="admin_tournament_history"))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="admin_panel"))
    
    await callback.message.edit_text("🏆 *Управление турнирами*", parse_mode="Markdown", reply_markup=builder.as_markup())
    await callback.answer()
...
async def admin_lottery_menu(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    if lottery["active"]:
        builder.row(InlineKeyboardButton(text="🎲 Завершить лотерею", callback_data="admin_end_lottery"))
        ...
        builder.row(InlineKeyboardButton(text="➕ Создать лотерею", callback_data="admin_create_lottery"))
        builder.row(InlineKeyboardButton(text="📜 История лотерей", callback_data="admin_lottery_history"))
...
async def admin_limits_menu(callback: CallbackQuery, state: FSMContext):
    ...
    text = (f"⚙️ *Управление ограничениями*\n\n"
            ...
            f"💰 Дневной лимит выводов: {settings.get('max_withdraw_per_day', 50000)} ⭐\n"
            f"⏰ Кулдаун выводов (минут): {settings.get('withdraw_cooldown_minutes', 60)}\n\n"
            f"Выберите параметр для изменения:")
    
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="📊 Дневной лимит потерь", callback_data="limit_daily_loss"))
    builder.row(InlineKeyboardButton(text="📈 Дневной лимит выигрыша", callback_data="limit_daily_win"))
    builder.row(InlineKeyboardButton(text="📉 Макс. проигрышей подряд", callback_data="limit_consecutive_losses"))
    builder.row(InlineKeyboardButton(text="💰 Мин. баланс для игры", callback_data="limit_min_balance"))
    builder.row(InlineKeyboardButton(text="💰 Дневной лимит выводов", callback_data="limit_withdraw_per_day"))
    builder.row(InlineKeyboardButton(text="⏰ Кулдаун выводов", callback_data="limit_withdraw_cooldown"))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="admin_panel"))
...
async def admin_limit_change(callback: CallbackQuery, state: FSMContext):
    ...
    names = {
        ...
        "withdraw_per_day": "дневной лимит выводов",
        "withdraw_cooldown": "кулдаун выводов в минутах"
    }
    await callback.message.edit_text(
...
async def admin_save_limit(message: Message, state: FSMContext):
    try:
        ...
        elif limit_type == "withdraw_per_day":
            settings["max_withdraw_per_day"] = value
        elif limit_type == "withdraw_cooldown":
            settings["withdraw_cooldown_minutes"] = value
...
async def admin_settings_menu(callback: CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="⭐ Стартовый баланс", callback_data="set_start_balance"),
               InlineKeyboardButton(text="💰 Мин. вывод", callback_data="set_min_withdraw"))
    builder.row(InlineKeyboardButton(text="👥 Реф. награда", callback_data="set_referral_reward"),
               InlineKeyboardButton(text="📊 Реф. процент", callback_data="set_referral_percent"))
    builder.row(InlineKeyboardButton(text="💱 Курс обмена", callback_data="set_exchange_rate"),
               InlineKeyboardButton(text="📦 Цена чек системы", callback_data="set_check_system_price"))
    builder.row(InlineKeyboardButton(text="🎲 Цена билета лотереи", callback_data="set_lottery_ticket_price"),
               InlineKeyboardButton(text="🎰 Казино", callback_data="set_casino"))
    builder.row(InlineKeyboardButton(text="🏆 Турниры", callback_data="set_tournaments"),
               InlineKeyboardButton(text="🔙 Назад", callback_data="admin_panel"))
