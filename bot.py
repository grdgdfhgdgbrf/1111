...
def build_vertical_keyboard_with_styles(buttons: List[Tuple[str, str, Optional[str]]]) -> InlineKeyboardMarkup:
    ...
    Args:
        buttons: Список кортежей (текст, callback_data, style).
...
async def cmd_show_profile(m: Message) -> None:
    ...
    Если указан аргумент (@user или ID), показывает профиль цели.
    Иначе показывает свой профиль.
    """
    row = db.fetch_one("SELECT * FROM players WHERE user_id=?", (m.from_user.id,))
    if not row:
...
# ==============================================================================
# 29. CALLBACK HANDLERS АРЕНЫ И ДУЭЛЕЙ
# ==============================================================================
...
async def main() -> None:
    ...
    logging.info("=" * 60)
    logging.info("⚔️ АРЕНА ДУЭЛЯНТОВ — Ultimate Edition v15.0")
    logging.info("=" * 60)
