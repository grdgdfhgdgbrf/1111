...
async def check_achievements(user_id: int):
    ...
    achievements = {
        ...
        "crash_master": {"name": "💥 Мастер Crash", "description": "Выиграть в Crash 100 раз",
                        "condition": user.get("crash_wins", 0) >= 100, "reward": 25000},
        "mines_master": {"name": "💣 Сапер", "description": "Выиграть в Mines 100 раз",
                        "condition": user.get("mines_wins", 0) >= 100, "reward": 25000}
    }
...
class CrashGame:
    def __init__(self, user_id: int, bet: int):
        ...
        self.house_edge = 0.05  # 5% преимущество казино
        
    def get_multiplier(self) -> float:
        """Получить текущий множитель"""
        elapsed = (datetime.now() - self.start_time).total_seconds()
        # Множитель растет по экспоненте, но с элементом случайности
        base_multiplier = 1 + (elapsed * 0.5)
        random_factor = random.uniform(0.8, 1.2)
        self.current_multiplier = base_multiplier * random_factor
        return self.current_multiplier
    
    def should_crash(self) -> bool:
        """Определить, должен ли краш произойти"""
        # Шанс краша увеличивается с ростом множителя
        crash_chance = min(0.95, (self.current_multiplier - 1) * 0.15 + self.house_edge)
...
class MinesGame:
    def __init__(self, user_id: int, bet: int, mines_count: int):
        ...
        self.size = 5  # Поле 5x5 = 25 клеток
        self.field = [[0 for _ in range(self.size)] for _ in range(self.size)]  # 0 - неизвестно, 1 - открыто, 2 - мина
    ...
    def _place_mines(self):
        """Разместить мины на поле"""
        total_cells = self.size * self.size
        positions = list(range(total_cells))
        random.shuffle(positions)
        
        for i in range(min(self.mines_count, total_cells)):
            pos = positions[i]
            row = pos // self.size
            col = pos % self.size
    ...
    def is_mine(self, row: int, col: int) -> bool:
        """Проверить, является ли клетка миной"""
        return (row, col) in self.mines_positions
    
    def open_cell(self, row: int, col: int) -> Tuple[bool, int]:
        """Открыть клетку"""
        if self.game_over:
            return False, 0, "Игра уже завершена!"
        
        if self.field[row][col] == 1:
            return False, 0, "Эта клетка уже открыта!"
        
        if self.is_mine(row, col):
            self.game_over = True
            self.won = False
            return False, 0, "💣 *Вы наступили на мину!*\nИгра окончена!"
        ...
        self.current_multiplier = 1 + (self.opened_cells / safe_cells) * 2
        ...
        if self.opened_cells >= safe_cells:
            ...
            self.won = True
            winnings = int(self.bet * self.current_multiplier)
            return True, winnings, f"🎉 *ПОБЕДА!*\nВы открыли все безопасные клетки!\nМножитель: {self.current_multiplier:.2f}x\nВыигрыш: {winnings} ⭐"
        
        return True, 0, f"✅ Клетка открыта!\nОткрыто клеток: {self.opened_cells}/{safe_cells}\nТекущий множитель: {self.current_multiplier:.2f}x"
    ...
    def get_field_display(self) -> str:
        """Получить визуальное отображение поля"""
        display = []
        for row in range(self.size):
            ...
            for col in range(self.size):
                if self.field[row][col] == 1:
                    row_display.append("✅")
                elif self.is_mine(row, col) and self.game_over:
                    row_display.append("💣")
                else:
                    row_display.append("⬜")
...
mines_games: Dict[int, MinesGame] = {}
...
GAMES = {
    ...
    "crash": {"name": "💥 Crash", "min_bet": 5, "max_bet": 5000},
    "mines": {"name": "💣 Mines", "min_bet": 10, "max_bet": 2000},
...
async def game_start(callback: CallbackQuery, state: FSMContext):
    ...
    if game_id == "crash":
        ...
        await callback.message.edit_text(
            ...
            f"• Вы можете забрать выигрыш в любой момент\n"
            f"• Если множитель крашится - вы теряете ставку\n\n"
        ...
        await callback.message.edit_text(
            ...
            f"💰 Ставки: от {game['min_bet']} до {game['max_bet']} ⭐\n\n"
            f"Как играть:\n"
            f"• Вы выбираете количество мин на поле 5x5\n"
            f"• Чем больше мин, тем выше множитель\n"
            f"• Открывайте безопасные клетки\n"
            f"• Множитель растет с каждым ходом\n"
...
# Обработчики Crash игры
@dp.message(CrashStates.waiting_for_bet)
async def crash_bet_handler(message: Message, state: FSMContext):
    try:
    ...
    await state.set_state(CrashStates.playing_game)
    
    game_obj = CrashGame(message.from_user.id, bet)
    crash_games[message.from_user.id] = game_obj
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💰 Забрать (1.00x)", callback_data="crash_cashout")]
    ])
    
    await message.answer(
        f"💥 *Игра Crash началась!*\n\n"
        f"Ставка: {bet} ⭐\n"
        f"Множитель: 1.00x\n\n"
        f"Множитель растет... Нажмите кнопку, чтобы забрать выигрыш!",
        parse_mode="Markdown",
        reply_markup=keyboard
    )
    
    # Запускаем обновление множителя
    asyncio.create_task(update_crash_multiplier(message.from_user.id, message.chat.id))

async def update_crash_multiplier(user_id: int, chat_id: int):
    """Обновление множителя в игре Crash"""
    ...
    for _ in range(100):  # Максимум 100 итераций
        ...
        if game.should_crash():
            ...
            try:
                await bot.send_message(
                    chat_id,
                    f"💥 *КРАШ!*\n\n"
                    f"Множитель достиг {multiplier:.2f}x и упал!\n"
        ...
        try:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=f"💰 Забрать ({multiplier:.2f}x)", callback_data="crash_cashout")]
            ])
...
@dp.callback_query(CrashStates.playing_game, F.data == "crash_cashout")
async def crash_cashout(callback: CallbackQuery, state: FSMContext):
    game = crash_games.get(callback.from_user.id)
    if not game:
    ...
    await add_stars(callback.from_user.id, winnings, f"Выигрыш в Crash (x{multiplier:.2f})")
    await update_user(callback.from_user.id, games_won=callback.from_user.id)
    await update_tournament_points(callback.from_user.id, winnings)
    
    del crash_games[callback.from_user.id]
...
async def mines_count_handler(message: Message, state: FSMContext):
    ...
    await message.answer(f"💰 Введите сумму ставки (от 10 до 2000 ⭐):")

@dp.message(MinesStates.waiting_for_bet)
async def mines_bet_handler(message: Message, state: FSMContext):
    try:
        bet = int(message.text)
    except ValueError:
        await message.answer("❌ Введите число!")
    ...
    await remove_stars(message.from_user.id, bet, f"Ставка в Mines ({mines_count} мин)")
    await update_user(message.from_user.id, games_played=user["games_played"] + 1)
    
    game_obj = MinesGame(message.from_user.id, bet, mines_count)
    mines_games[message.from_user.id] = game_obj
    ...
    await message.answer(
        ...
        f"Безопасных клеток: {safe_cells}\n"
        f"Текущий множитель: 1.00x\n\n"
        f"{game_obj.get_field_display()}\n\n"
        f"Открывайте клетки, нажимая на кнопки!",
        parse_mode="Markdown",
        reply_markup=keyboard
    )
    
    # Создаем поле с кнопками
    await update_mines_field(message.from_user.id, message.chat.id)

async def update_mines_field(user_id: int, chat_id: int):
    """Обновление поля Mines"""
    ...
    for row in range(5):
        ...
        for col in range(5):
            if game.field[row][col] == 1:
                row_buttons.append(InlineKeyboardButton(text="✅", callback_data=f"mines_cell_{row}_{col}_opened"))
            else:
                row_buttons.append(InlineKeyboardButton(text="⬜", callback_data=f"mines_cell_{row}_{col}"))
        buttons.append(row_buttons)
    
    buttons.append([InlineKeyboardButton(text="💰 Забрать выигрыш", callback_data="mines_cashout")])
    buttons.append([InlineKeyboardButton(text="❌ Завершить игру", callback_data="mines_end")])
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
    
    safe_cells = 25 - game.mines_count
    ...
    try:
        await bot.edit_message_text(
            ...
            f"Открыто клеток: {game.opened_cells}/{safe_cells}\n"
...
@dp.callback_query(MinesStates.playing_game, F.data.startswith("mines_cell_"))
async def mines_open_cell(callback: CallbackQuery, state: FSMContext):
    ...
    success, winnings, result_text = await game.open_cell(row, col)
    
    if not success:
        if "мину" in result_text:
            # Игрок проиграл
            await callback.message.edit_text(
                result_text,
                parse_mode="Markdown",
            ...
            await update_tournament_points(callback.from_user.id, game.bet)
    ...
    if winnings > 0:
        # Игрок выиграл (открыл все клетки)
        await add_stars(callback.from_user.id, winnings, f"Выигрыш в Mines (x{game.current_multiplier:.2f})")
        await update_user(callback.from_user.id, games_won=callback.from_user.id)
        await update_tournament_points(callback.from_user.id, winnings)
    ...
    # Обновляем поле
    await update_mines_field(callback.from_user.id, callback.message.chat.id)
    await callback.answer(result_text, show_alert=True)

@dp.callback_query(MinesStates.playing_game, F.data == "mines_cashout")
async def mines_cashout(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"✅ *Вы забрали выигрыш!*\n\n"
        f"Ставка: {game.bet} ⭐\n"
        f"Открыто клеток: {game.opened_cells}\n"
...
@dp.callback_query(MinesStates.playing_game, F.data == "mines_end")
async def mines_end(callback: CallbackQuery, state: FSMContext):
    game = mines_games.get(callback.from_user.id)
    if game:
        del mines_games[callback.from_user.id]
...
# Остальные обработчики игр (аналогично предыдущей версии)
...
async def show_help(callback: CallbackQuery):
    ...
    text = (f"❓ *Помощь*\n\n"
            ...
            f"💥 *Crash:* Ставка растет с множителем, нужно забрать до краха\n"
            f"💣 *Mines:* Поле 5x5 с минами, чем больше мин - тем выше множитель\n\n"
