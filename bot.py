...
# Система чеков (доступна всем пользователям)
async def create_any_check(user_id: int, amount: int) -> Tuple[bool, str, str]:
    """Создание чека любым пользователем"""
    settings = await load_json(SETTINGS_FILE, {})
    min_check = 100
    max_check = 100000
...
# Система промокодов (только админ)
...
async def check_task_completion(user_id: int, task: dict) -> bool:
    """Проверка выполнения задания с проверкой бота в канале"""
    ...
    try:
        ...
        # Если есть требуемый бот, проверяем его наличие в канале
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                ...
                f"Для ответа используйте: /reply_{ticket_id} <текст>",
                parse_mode="Markdown"
            )
        except:
            pass
...
async def reply_to_ticket(admin_id: int, ticket_id: int, message: str) -> bool:
    ...
    try:
        await bot.send_message(
            ...
            f"💬 {message}\n\n"
            f"Для ответа используйте /support",
            parse_mode="Markdown"
        )
    except:
        pass
...
# Игры с реалистичными шансами
GAMES = {
    "coinflip": {"name": "🎲 Орёл или Решка", "min_bet": 1, "max_bet": 1000, "win_chance": 49.5, "multiplier": 2},
    "dice": {"name": "🎲 Кости", "min_bet": 1, "max_bet": 500, "win_chance": 16.67, "multiplier": 5},
    "rps": {"name": "✊ КНБ", "min_bet": 1, "max_bet": 500, "win_chance": 33.33, "multiplier": 2.8},
    "roulette": {"name": "🎡 Рулетка", "min_bet": 5, "max_bet": 2000, "win_chance": 48.6, "multiplier": 2},
    "poker": {"name": "🃏 Покер", "min_bet": 10, "max_bet": 5000, "win_chance": 42, "multiplier": 2.2},
    "baccarat": {"name": "🎴 Баккара", "min_bet": 10, "max_bet": 5000, "win_chance": 44.6, "multiplier": 2}
}
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    text = (f"✨ Добро пожаловать, {message.from_user.full_name}!\n\n"
            ...
            f"🎮 6 игр с реалистичными шансами\n"
            f"🎰 Казино | 🏆 Турниры\n"
            f"📋 Задания на каналы\n"
            f"👥 Рефералы (10% от покупок)\n"
            f"📦 Создавайте и активируйте чеки\n"
...
async def games_menu(callback: CallbackQuery):
    text = "🎮 *Игры*\n\nВыберите игру (шансы на победу указаны):"
    buttons = []
    for game_id, game in GAMES.items():
        buttons.append([InlineKeyboardButton(text=f"{game['name']} ({game['win_chance']}%)", callback_data=f"game_{game_id}")])
...
async def game_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"🎮 *{game['name']}*\n\n"
        f"📊 Шанс на победу: {game['win_chance']}%\n"
...
async def casino_menu(callback: CallbackQuery):
    ...
    text = "🎰 *Казино*\n\nВыберите игру:"
    buttons = [
        [InlineKeyboardButton(text="🎰 Слоты (x1.5-x8)", callback_data="casino_slots"),
         InlineKeyboardButton(text="🎲 Кости (x5.5)", callback_data="casino_dice")],
...
async def create_check_menu(callback: CallbackQuery, state: FSMContext):
...
async def create_check_amount(message: Message, state: FSMContext):
    ...
    success, msg, code = await create_any_check(message.from_user.id, amount)
...
async def support_send_reply(message: Message, state: FSMContext):
    ...
    if ticket_id:
        # Отправляем ответ в существующий тикет
...
async def show_help(callback: CallbackQuery):
    ...
    text = (f"❓ *Помощь*\n\n"
            f"🎮 *Игры:*\n"
            f"• Орёл/Решка (49.5%) | Кости (16.67%) | КНБ (33.33%)\n"
            f"• Рулетка (48.6%) | Покер (42%) | Баккара (44.6%)\n\n"
            f"🎰 *Казино:*\n"
            f"• Слоты (x1.5-x8) | Кости (x5.5)\n\n"
            f"🏆 *Турниры:*\n"
            f"• Участвуйте в турнирах, получайте очки, выигрывайте призы\n\n"
            f"📋 *Задания:*\n"
            ...
            f"📦 *Чеки:*\n"
            f"• Создавайте чеки для друзей (от 100 ⭐)\n"
