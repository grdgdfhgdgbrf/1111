...
async def init_data():
    ...
    bonuses = await load_json(BONUSES_FILE, {"active": []})
...
async def check_achievements(user_id: int):
    ...
    achievements = {
        ...
        "streak_7": {"name": "📅 Недельный стрик", "description": "Получить ежедневный бонус 7 дней подряд",
                    "condition": user["daily_bonus_streak"] >= 7, "reward": 5000},
        "streak_30": {"name": "🏆 Месячный стрик", "description": "Получить ежедневный бонус 30 дней подряд",
...
async def claim_daily_bonus(user_id: int) -> Tuple[bool, int, str]:
    ...
    if last_bonus:
        ...
        if last_date == today:
            return False, 0, "Вы уже получили ежедневный бонус сегодня!"
...
async def claim_hourly_bonus(user_id: int) -> Tuple[bool, int, str]:
    user = await get_user(user_id)
    settings = await load_json(SETTINGS_FILE, {})
    
    if not settings.get("hourly_bonus_enabled", True):
        return False, 0, "Часовой бонус отключен администратором!"
    
    last_bonus = user.get("last_hourly_bonus")
    now = datetime.now()
    ...
    if last_bonus:
        ...
        if (now - last_time).total_seconds() < 3600:
            ...
            return False, 0, f"Следующий бонус через {minutes} минут!"
    
    bonus = random.randint(10, 50)
    await add_stars(user_id, bonus, f"Часовой бонус")
    await update_user(user_id, last_hourly_bonus=now.isoformat())
    
    return True, bonus, f"⭐ Получено: {bonus} звезд"

async def claim_weekly_bonus(user_id: int) -> Tuple[bool, int, str]:
    user = await get_user(user_id)
    settings = await load_json(SETTINGS_FILE, {})
    
    if not settings.get("weekly_bonus_enabled", True):
        return False, 0, "Еженедельный бонус отключен администратором!"
    
    last_bonus = user.get("last_weekly_bonus")
    now = datetime.now()
    
    if last_bonus:
        last_time = datetime.fromisoformat(last_bonus)
        if (now - last_time).days < 7:
            remaining = 7 - (now - last_time).days
            return False, 0, f"Следующий бонус через {remaining} дней!"
    
    streak = user.get("daily_bonus_streak", 0)
    bonus = min(50 + streak * 10, 1000)
    await add_stars(user_id, bonus, f"Еженедельный бонус")
    await update_user(user_id, last_weekly_bonus=now.isoformat())
    
    return True, bonus, f"🔥 Стрик: {streak} дней!\n⭐ Получено: {bonus} звезд"

# Система ограничений
async def check_user_limit(user_id: int, game: str, bet: int) -> Tuple[bool, str]:
    limits = await load_json(LIMITS_FILE, {"user_limits": {}, "game_limits": {}})
    user_limits = limits["user_limits"].get(str(user_id), {"daily_loss": 0, "daily_win": 0, "last_reset": None})
    game_limits = limits["game_limits"].get(game, {"max_bet": 10000, "min_bet": 1, "daily_limit": 100000})
    
    last_reset = user_limits.get("last_reset")
    if not last_reset or datetime.fromisoformat(last_reset).date() != datetime.now().date():
        user_limits = {"daily_loss": 0, "daily_win": 0, "last_reset": datetime.now().isoformat()}
    
    if bet > game_limits.get("max_bet", 10000):
        return False, f"Максимальная ставка для {game}: {game_limits['max_bet']} ⭐"
    
    if bet < game_limits.get("min_bet", 1):
        return False, f"Минимальная ставка для {game}: {game_limits['min_bet']} ⭐"
    
    if user_limits["daily_loss"] + bet > game_limits.get("daily_limit", 100000):
        return False, f"Дневной лимит потерь: {game_limits['daily_limit']} ⭐"
    
    return True, "OK"

async def update_user_limit(user_id: int, amount: int, is_loss: bool = True):
    limits = await load_json(LIMITS_FILE, {"user_limits": {}, "game_limits": {}})
    user_limits = limits["user_limits"].get(str(user_id), {"daily_loss": 0, "daily_win": 0, "last_reset": None})
    ...
    if is_loss:
        user_limits["daily_loss"] += amount
    else:
        user_limits["daily_win"] += amount
    
    limits["user_limits"][str(user_id)] = user_limits
    await save_json(LIMITS_FILE, limits)
...
class MinesGame:
    ...
    def get_multiplier(self) -> float:
        ...
        if opened_safe == 0:
            return 1.0
        multiplier = 1.0 + (opened_safe / safe_cells) * 4.0
...
# Казино игры
CASINO_GAMES = {
    "slots": {"name": "🎰 Слоты", "min_bet": 10, "max_bet": 5000},
    "dice": {"name": "🎲 Кости", "min_bet": 5, "max_bet": 5000},
    "keno": {"name": "🎯 Кено", "min_bet": 10, "max_bet": 2000},
    "wheel": {"name": "🎡 Колесо Фортуны", "min_bet": 5, "max_bet": 1500},
    "hilo": {"name": "⬆️ Выше/Ниже", "min_bet": 5, "max_bet": 1000},
    "plinko": {"name": "⚫ Plinko", "min_bet": 10, "max_bet": 2000},
...
async def play_coinflip(user_id: int, bet: int, choice: str) -> Tuple[bool, int, str]:
    ...
    if win:
        ...
        await update_user_limit(user_id, winnings, False)
        ...
        await update_user_limit(user_id, bet, True)
...
async def play_rps(user_id: int, bet: int, choice: str) -> Tuple[bool, int, str]:
    ...
    if choice == bot_choice:
        ...
        await update_user_limit(user_id, bet, False)
...
async def play_plinko(user_id: int, bet: int, rows: int = 16) -> Tuple[bool, int, str]:
...
async def casino_keno(user_id: int, bet: int, numbers: List[int]) -> Tuple[bool, int, str]:
    ...
    if multiplier > 0:
        winnings = int(bet * multiplier)
        await add_stars(user_id, winnings, f"Выигрыш в Кено (казино)")
        user = await get_user(user_id)
        await update_user(user_id, casino_wins=user["casino_wins"] + 1)
        await update_tournament_points(user_id, winnings)
        await update_user_limit(user_id, winnings, False)
...
async def casino_wheel(user_id: int, bet: int) -> Tuple[bool, int, str]:
    ...
    if multiplier > 0:
        winnings = int(bet * multiplier)
        await add_stars(user_id, winnings, f"Выигрыш в Колесе Фортуны (казино)")
        user = await get_user(user_id)
        await update_user(user_id, casino_wins=user["casino_wins"] + 1)
        await update_tournament_points(user_id, winnings)
        await update_user_limit(user_id, winnings, False)
...
async def casino_hilo(user_id: int, bet: int, choice: str, current_card: int) -> Tuple[bool, int, str, int]:
    ...
    if win:
        winnings = int(bet * 1.9)
        await add_stars(user_id, winnings, f"Выигрыш в Выше/Ниже (казино)")
        user = await get_user(user_id)
        await update_user(user_id, casino_wins=user["casino_wins"] + 1)
        await update_tournament_points(user_id, winnings)
        await update_user_limit(user_id, winnings, False)
...
async def casino_plinko(user_id: int, bet: int) -> Tuple[bool, int, str]:
    ...
    if result > 1:
        winnings = int(bet * result)
        await add_stars(user_id, winnings, f"Выигрыш в Plinko (казино)")
        user = await get_user(user_id)
        await update_user(user_id, casino_wins=user["casino_wins"] + 1)
        await update_tournament_points(user_id, winnings)
        await update_user_limit(user_id, winnings, False)
...
async def casino_jackpot(user_id: int, bet: int) -> Tuple[bool, int, str]:
    ...
    if win:
        winnings = int(bet * 100)
        await add_stars(user_id, winnings, f"Выигрыш в Джекпот (казино)")
        user = await get_user(user_id)
        await update_user(user_id, casino_wins=user["casino_wins"] + 1)
        await update_tournament_points(user_id, winnings)
        await update_user_limit(user_id, winnings, False)
...
async def crash_bet(message: Message, state: FSMContext):
    ...
    limit_ok, limit_msg = await check_user_limit(message.from_user.id, "crash", bet)
    if not limit_ok:
...
async def update_crash_multiplier(user_id: int, state: FSMContext, message_id: int):
    ...
    while crash_game.is_active:
        ...
        if not crash_game.is_active:
            ...
            await update_user_limit(user_id, crash_game.bet, True)
            await state.clear()
            return
        else:
...
async def mines_bet(message: Message, state: FSMContext):
    ...
    limit_ok, limit_msg = await check_user_limit(message.from_user.id, "mines", bet)
    if not limit_ok:
        await message.answer(f"❌ {limit_msg}")
        return
...
async def show_mines_field(message: Message, state: FSMContext, mines_game: MinesGame):
    ...
    buttons.append([InlineKeyboardButton(text="🔙 Выход", callback_data="games_menu")])
...
async def mines_open_cell(callback: CallbackQuery, state: FSMContext):
    ...
    if not is_safe:
        ...
        await update_user_limit(callback.from_user.id, mines_game.bet, True)
        await state.clear()
...
async def process_bet(message: Message, state: FSMContext):
    ...
    limit_ok, limit_msg = await check_user_limit(message.from_user.id, game_id, bet)
    if not limit_ok:
        await message.answer(f"❌ {limit_msg}")
        return
...
async def casino_menu(callback: CallbackQuery):
    ...
    for game_id, game in CASINO_GAMES.items():
        buttons.append([InlineKeyboardButton(text=game['name'], callback_data=f"casino_{game_id}")])
    buttons.append([InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_main")])
...
async def casino_start(callback: CallbackQuery, state: FSMContext):
    ...
    game_data = CASINO_GAMES[game]
    
    await callback.message.edit_text(
        f"🎰 *{game_data['name']}*\n\n"
        f"💰 Ставки: от {game_data['min_bet']} до {game_data['max_bet']} ⭐\n\n"
        f"Введите сумму ставки:",
        parse_mode="Markdown",
...
async def casino_bet(message: Message, state: FSMContext):
    ...
    limit_ok, limit_msg = await check_user_limit(message.from_user.id, game, bet)
    if not limit_ok:
        await message.answer(f"❌ {limit_msg}")
        return
    ...
    await remove_stars(message.from_user.id, bet, f"Ставка в казино ({game})")
    ...
    if game == "slots":
        ...
        await state.set_state(CasinoStates.waiting_hilo_choice)
        current_card = random.randint(1, 13)
        await state.update_data(current_card=current_card)
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⬆️ Выше", callback_data="hilo_higher"),
        ...
        win, winnings, result_text = await casino_plinko(message.from_user.id, bet)
        await message.answer(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⚫ Ещё раз", callback_data="casino_plinko"),
             InlineKeyboardButton(text="🔙 В меню", callback_data="casino_menu")]
        ]))
        await state.clear()
    
    elif game == "jackpot":
        win, winnings, result_text = await casino_jackpot(message.from_user.id, bet)
        await message.answer(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💰 Ещё раз", callback_data="casino_jackpot"),
             InlineKeyboardButton(text="🔙 В меню", callback_data="casino_menu")]
        ]))
        await state.clear()
...
async def casino_keno_choice(message: Message, state: FSMContext):
    try:
        numbers = [int(x) for x in message.text.split()]
        if len(numbers) != 5 or not all(1 <= n <= 80 for n in numbers):
            await message.answer("❌ Введите 5 чисел от 1 до 80 через пробел!")
    ...
    win, winnings, result_text = await casino_keno(message.from_user.id, bet, numbers)
    await message.answer(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎯 Ещё раз", callback_data="casino_keno"),
         InlineKeyboardButton(text="🔙 В меню", callback_data="casino_menu")]
    ]))
    await state.clear()

@dp.callback_query(CasinoStates.waiting_hilo_choice, F.data.startswith("hilo_"))
async def casino_hilo_choice(callback: CallbackQuery, state: FSMContext):
    choice = callback.data.replace("hilo_", "")
    data = await state.get_data()
    bet = data["casino_bet"]
    current_card = data.get("current_card", 7)
    
    win, winnings, result_text, next_card = await casino_hilo(callback.from_user.id, bet, choice, current_card)
    
    await callback.message.edit_text(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🃏 Ещё раз", callback_data="casino_hilo"),
         InlineKeyboardButton(text="🔙 В меню", callback_data="casino_menu")]
    ]))
    await state.clear()
    await callback.answer()
...
async def bonus_menu(callback: CallbackQuery):
    ...
    daily_status = "✅ Доступен" if settings.get("daily_bonus_enabled", True) else "❌ Отключен"
    hourly_status = "✅ Доступен" if settings.get("hourly_bonus_enabled", True) else "❌ Отключен"
    weekly_status = "✅ Доступен" if settings.get("weekly_bonus_enabled", True) else "❌ Отключен"
    
    daily_streak = user.get("daily_bonus_streak", 0)
    daily_bonus = min(5 + daily_streak * 5, 500)
    
    text = (f"🎁 *Бонусы*\n\n"
            f"📅 *Ежедневный бонус* - {daily_status}\n"
            f"└ Награда: {daily_bonus} ⭐ (стрик: {daily_streak} дней)\n\n"
            f"⏰ *Часовой бонус* - {hourly_status}\n"
            f"└ Награда: 10-50 ⭐ (каждый час)\n\n"
            f"📆 *Еженедельный бонус* - {weekly_status}\n"
            f"└ Награда: 50-1000 ⭐ (раз в неделю)\n\n"
            f"Выберите бонус для получения:")
    
    buttons = [
        [InlineKeyboardButton(text="📅 Ежедневный бонус", callback_data="claim_daily")],
        [InlineKeyboardButton(text="⏰ Часовой бонус", callback_data="claim_hourly")],
        [InlineKeyboardButton(text="📆 Еженедельный бонус", callback_data="claim_weekly")],
...
@dp.callback_query(F.data == "claim_hourly")
async def claim_hourly(callback: CallbackQuery):
    success, amount, msg = await claim_hourly_bonus(callback.from_user.id)
...
async def claim_weekly(callback: CallbackQuery):
    success, amount, msg = await claim_weekly_bonus(callback.from_user.id)
...
async def show_help(callback: CallbackQuery):
    settings = await load_json(SETTINGS_FILE, {})
    text = (f"❓ *Помощь*\n\n"
            f"🎮 *Игры (16):* Орёл/Решка, Кости, КНБ, Рулетка, Покер, Баккара, Блэкджек, Crash, Mines, Кено, Колесо Фортуны, Выше/Ниже, Хай-Ло, Plinko, Дуэль костей, Джекпот\n\n"
            ...
            f"🎰 *Казино (7):* Слоты, Кости, Кено, Колесо Фортуны, Выше/Ниже, Plinko, Джекпот\n\n"
            f"🎁 *Бонусы:* Ежедневный, часовой и еженедельный бонусы\n\n"
...
# Управление бонусами
@dp.callback_query(F.data == "admin_bonuses")
async def admin_bonuses_menu(callback: CallbackQuery, state: FSMContext):
    ...
    text = (f"🎁 *Управление бонусами*\n\n"
            f"📅 Ежедневный бонус: {'✅ Вкл' if settings.get('daily_bonus_enabled', True) else '❌ Выкл'}\n"
            f"⏰ Часовой бонус: {'✅ Вкл' if settings.get('hourly_bonus_enabled', True) else '❌ Выкл'}\n"
            f"📆 Еженедельный бонус: {'✅ Вкл' if settings.get('weekly_bonus_enabled', True) else '❌ Выкл'}\n\n"
            f"Выберите действие:")
    
    buttons = [
        [InlineKeyboardButton(text="📅 Ежедневный бонус", callback_data="admin_toggle_daily")],
        [InlineKeyboardButton(text="⏰ Часовой бонус", callback_data="admin_toggle_hourly")],
        [InlineKeyboardButton(text="📆 Еженедельный бонус", callback_data="admin_toggle_weekly")],
        [InlineKeyboardButton(text="➕ Создать специальный бонус", callback_data="admin_create_bonus")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_panel")]
    ]
    await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await callback.answer()
...
async def admin_toggle_bonus(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("❌ Недостаточно прав!")
        return
    
    bonus = callback.data.replace("admin_toggle_", "")
    settings = await load_json(SETTINGS_FILE, {})
    
    if bonus == "daily":
        settings["daily_bonus_enabled"] = not settings.get("daily_bonus_enabled", True)
    elif bonus == "hourly":
        settings["hourly_bonus_enabled"] = not settings.get("hourly_bonus_enabled", True)
    elif bonus == "weekly":
        settings["weekly_bonus_enabled"] = not settings.get("weekly_bonus_enabled", True)
    
    await save_json(SETTINGS_FILE, settings)
    await callback.answer("✅ Настройка обновлена!", show_alert=True)
    await admin_bonuses_menu(callback, None)
...
async def admin_create_bonus(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🎁 Введите сумму бонуса для всех пользователей:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Отмена", callback_data="admin_bonuses")]
    ]))
    await callback.answer()

@dp.message(AdminStates.waiting_for_bonus_amount)
async def admin_process_bonus(message: Message, state: FSMContext):
    try:
        amount = int(message.text)
    except ValueError:
        await message.answer("❌ Введите число!")
    ...
    await state.set_state(AdminStates.waiting_for_bonus_limit)
    await state.update_data(bonus_amount=amount)
    await message.answer("👥 Введите лимит пользователей (0 - для всех):")

@dp.message(AdminStates.waiting_for_bonus_limit)
async def admin_process_bonus_limit(message: Message, state: FSMContext):
    try:
        limit = int(message.text)
    except ValueError:
        await message.answer("❌ Введите число!")
    ...
    if limit == 0:
        for user_id in users:
            await add_stars(int(user_id), amount, f"Специальный бонус от админа {message.from_user.id}")
            count += 1
    else:
        for user_id in list(users.keys())[:limit]:
            await add_stars(int(user_id), amount, f"Специальный бонус от админа {message.from_user.id}")
            count += 1
    
    await message.answer(f"✅ Бонус выдан {count} пользователям по {amount} ⭐")
    await log_admin_action(message.from_user.id, "create_bonus", None, f"Amount: {amount}, Users: {count}")
    await state.clear()
...
async def admin_limits_menu(callback: CallbackQuery, state: FSMContext):
    ...
    limits = await load_json(LIMITS_FILE, {"user_limits": {}, "game_limits": {}})
    game_limits = limits.get("game_limits", {})
    
    text = "⚙️ *Управление ограничениями*\n\n"
    text += "*Текущие ограничения игр:*\n"
    for game, limit in game_limits.items():
        text += f"└ {game}: макс. ставка {limit.get('max_bet', 10000)} ⭐\n"
    text += "\nВыберите действие:"
    
    buttons = [
        [InlineKeyboardButton(text="🎮 Установить лимит игры", callback_data="admin_set_game_limit")],
        [InlineKeyboardButton(text="👤 Установить лимит пользователя", callback_data="admin_set_user_limit")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_panel")]
    ]
    await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await callback.answer()

@dp.callback_query(F.data == "admin_set_game_limit")
async def admin_set_game_limit(callback: CallbackQuery, state: FSMContext):
    if not await is_admin(callback.from_user.id):
        await callback.answer("❌ Недостаточно прав!")
        return
    
    games_list = "\n".join([f"• {g}" for g in list(GAMES.keys()) + list(CASINO_GAMES.keys())])
    await state.set_state(AdminStates.waiting_for_setting)
    await state.update_data(setting_type="game_limit")
    await callback.message.edit_text(
        f"🎮 Введите название игры и максимальную ставку через пробел\n\n"
        f"Доступные игры:\n{games_list}\n\n"
        f"Пример: slots 5000",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
@dp.callback_query(F.data == "admin_set_user_limit")
async def admin_set_user_limit(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminStates.waiting_for_user_id)
    await state.update_data(setting_type="user_limit")
    await callback.message.edit_text(
...
@dp.message(AdminStates.waiting_for_setting)
async def admin_process_game_limit(message: Message, state: FSMContext):
    data = await state.get_data()
    setting_type = data.get("setting_type")
    
    if setting_type == "game_limit":
        try:
            ...
            limits = await load_json(LIMITS_FILE, {"user_limits": {}, "game_limits": {}})
            if "game_limits" not in limits:
                limits["game_limits"] = {}
            
            limits["game_limits"][game] = {
                "max_bet": max_bet,
                "min_bet": 1,
                "daily_limit": 100000
            }
            await save_json(LIMITS_FILE, limits)
            await message.answer(f"✅ Лимит для игры {game} установлен: {max_bet} ⭐")
            await log_admin_action(message.from_user.id, "set_game_limit", None, f"Game: {game}, Limit: {max_bet}")
        except:
            await message.answer("❌ Неверный формат! Используйте: <игра> <макс. ставка>")
    
    await state.clear()

@dp.message(AdminStates.waiting_for_user_id)
async def admin_set_user_limit_id(message: Message, state: FSMContext):
    try:
        user_id = int(message.text)
        await state.update_data(target_user=user_id)
        await state.set_state(AdminStates.waiting_for_limit_value)
        await message.answer("💰 Введите дневной лимит потерь для пользователя:")
    except ValueError:
        await message.answer("❌ Введите корректный ID!")
        await state.clear()

@dp.message(AdminStates.waiting_for_limit_value)
async def admin_process_user_limit(message: Message, state: FSMContext):
    try:
        limit = int(message.text)
        data = await state.get_data()
        user_id = data["target_user"]
        ...
        limits = await load_json(LIMITS_FILE, {"user_limits": {}, "game_limits": {}})
        if "user_limits" not in limits:
            limits["user_limits"] = {}
        
        if str(user_id) not in limits["user_limits"]:
            limits["user_limits"][str(user_id)] = {"daily_loss": 0, "daily_win": 0, "last_reset": None}
        
        limits["user_limits"][str(user_id)]["daily_limit"] = limit
        await save_json(LIMITS_FILE, limits)
        
        await message.answer(f"✅ Пользователю {user_id} установлен дневной лимит: {limit} ⭐")
        await log_admin_action(message.from_user.id, "set_user_limit", str(user_id), f"Limit: {limit}")
    except ValueError:
        await message.answer("❌ Введите число!")
