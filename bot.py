...
class CasinoStates(StatesGroup):
    ...
    waiting_sicbo_bet = State()
    waiting_war_choice = State()
    waiting_andar_bahar_choice = State()
...
class AdminStates(StatesGroup):
    ...
    waiting_for_withdrawal_cooldown = State()
...
async def get_user(user_id: int) -> dict:
    ...
    if user.get("last_reset"):
        ...
        if last_reset.date() != datetime.now().date():
            user["daily_loss"] = 0
            user["daily_win"] = 0
            user["withdrawals_today"] = 0
            user["last_reset"] = datetime.now().isoformat()
            await update_user(user_id, daily_loss=0, daily_win=0, withdrawals_today=0, last_reset=datetime.now().isoformat())
    else:
...
# Система вывода с кулдауном
async def check_withdrawal_limit(user_id: int) -> Tuple[bool, str]:
    user = await get_user(user_id)
    settings = await load_json(SETTINGS_FILE, {})
    cooldown_hours = settings.get("withdrawal_cooldown_hours", 24)
    max_per_day = settings.get("max_withdrawals_per_day", 3)
    
    # Проверка лимита на день
    if user["withdrawals_today"] >= max_per_day:
        return False, f"Вы достигли лимита выводов на сегодня ({max_per_day} выводов)"
    
    # Проверка кулдауна
    if user["last_withdrawal"]:
        last_withdrawal = datetime.fromisoformat(user["last_withdrawal"])
        next_withdrawal = last_withdrawal + timedelta(hours=cooldown_hours)
        if datetime.now() < next_withdrawal:
            time_left = next_withdrawal - datetime.now()
            hours = time_left.seconds // 3600
            minutes = (time_left.seconds % 3600) // 60
            return False, f"Следующий вывод доступен через {hours}ч {minutes}м"
    
    return True, "OK"

async def register_withdrawal(user_id: int):
    user = await get_user(user_id)
    await update_user(user_id, last_withdrawal=datetime.now().isoformat(), withdrawals_today=user["withdrawals_today"] + 1)
...
class CrashGame:
    ...
    async def update_multiplier(self) -> float:
        crash_probability = 1 / (self.current_multiplier * 5)
        if random.random() < crash_probability:
            self.is_active = False
            self.crashed_at = self.current_multiplier
            return 0
        self.current_multiplier += random.uniform(0.02, 0.1)
...
GAMES = {
    ...
    "war": {"name": "⚔️ Война", "min_bet": 10, "max_bet": 2000, "multiplier": 1.9},
    "sicbo": {"name": "🎲 СИК БО", "min_bet": 10, "max_bet": 3000, "multiplier": 6},
    "andar_bahar": {"name": "🌿 Андар Баар", "min_bet": 10, "max_bet": 2000, "multiplier": 1.9}
}
...
CASINO_GAMES = {
    ...
    "war": {"name": "⚔️ Война", "min_bet": 10, "max_bet": 2000},
    "sicbo": {"name": "🎲 СИК БО", "min_bet": 10, "max_bet": 3000},
    "andar_bahar": {"name": "🌿 Андар Баар", "min_bet": 10, "max_bet": 2000}
}
...
async def play_war(user_id: int, bet: int) -> Tuple[bool, int, str]:
...
async def play_sicbo(user_id: int, bet: int, choice: str) -> Tuple[bool, int, str]:
    ...
    if win:
        winnings = int(bet * multiplier)
        await add_stars(user_id, winnings, f"Выигрыш в СИК БО")
        await update_tournament_points(user_id, winnings)
        user = await get_user(user_id)
        await update_user(user_id, games_won=user["games_won"] + 1)
        await update_user_limits(user_id, winnings, is_win=True)
        return True, winnings, f"🎲 Кости: {dice1}, {dice2}, {dice3}\n📊 Сумма: {total}\n\n🎉 Вы выиграли {winnings} ⭐!"
    else:
        await update_tournament_points(user_id, bet)
        await update_user_limits(user_id, bet, is_loss=True)
        return False, bet, f"🎲 Кости: {dice1}, {dice2}, {dice3}\n📊 Сумма: {total}\n\n😔 Проигрыш: {bet} ⭐"

async def play_andar_bahar(user_id: int, bet: int, choice: str) -> Tuple[bool, int, str]:
    joker = random.randint(1, 13)
    andar_count = 0
    bahar_count = 0
    ...
    if win:
        winnings = int(bet * 1.9)
        await add_stars(user_id, winnings, f"Выигрыш в Андар Баар")
        await update_tournament_points(user_id, winnings)
        user = await get_user(user_id)
        await update_user(user_id, games_won=user["games_won"] + 1)
        await update_user_limits(user_id, winnings, is_win=True)
        return True, winnings, f"🌿 Джокер: {joker}\n🎲 Выпало: {result.upper()}\n\n🎉 Вы выиграли {winnings} ⭐!"
    else:
        await update_tournament_points(user_id, bet)
        await update_user_limits(user_id, bet, is_loss=True)
        return False, bet, f"🌿 Джокер: {joker}\n🎲 Выпало: {result.upper()}\n\n😔 Проигрыш: {bet} ⭐"
...
async def casino_war(user_id: int, bet: int) -> Tuple[bool, int, str]:
    player_card = random.randint(1, 13)
    dealer_card = random.randint(1, 13)
...
async def casino_sicbo(user_id: int, bet: int, choice: str) -> Tuple[bool, int, str]:
    dice1 = random.randint(1, 6)
    dice2 = random.randint(1, 6)
    dice3 = random.randint(1, 6)
    total = dice1 + dice2 + dice3
    ...
    if win:
        winnings = int(bet * multiplier)
        await add_stars(user_id, winnings, f"Выигрыш в СИК БО (казино)")
        user = await get_user(user_id)
        await update_user(user_id, casino_wins=user["casino_wins"] + 1)
        await update_tournament_points(user_id, winnings)
        await update_user_limits(user_id, winnings, is_win=True)
...
async def casino_andar_bahar(user_id: int, bet: int, choice: str) -> Tuple[bool, int, str]:
    joker = random.randint(1, 13)
    andar_count = 0
    bahar_count = 0
    ...
    if win:
        winnings = int(bet * 1.9)
        await add_stars(user_id, winnings, f"Выигрыш в Андар Баар (казино)")
        user = await get_user(user_id)
        await update_user(user_id, casino_wins=user["casino_wins"] + 1)
        await update_tournament_points(user_id, winnings)
        await update_user_limits(user_id, winnings, is_win=True)
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    text = (f"✨ Добро пожаловать, {message.from_user.full_name}!\n\n"
            ...
            f"🎮 22 игры | 🎰 12 игр в казино | 🏆 Турниры\n"
            ...
            f"⚔️ War - карточная война с дилером!\n"
            f"🎲 Sic Bo - китайские кости!\n"
            f"🌿 Andar Bahar - индийская карточная игра!\n"
            ...
            f"⏱️ Кулдаун между выводами: {settings.get('withdrawal_cooldown_hours', 24)} часов\n"
            f"📊 Макс. выводов в день: {settings.get('max_withdrawals_per_day', 3)}\n\n"
            f"⚠️ *Лимиты:*\n"
...
async def games_menu(callback: CallbackQuery):
    ...
    # Разбиваем игры на ряды по 2 кнопки
    games_list = list(GAMES.items())
    for i in range(0, len(games_list), 2):
        row = []
        row.append(InlineKeyboardButton(text=games_list[i][1]['name'], callback_data=f"game_{games_list[i][0]}"))
        if i + 1 < len(games_list):
            row.append(InlineKeyboardButton(text=games_list[i+1][1]['name'], callback_data=f"game_{games_list[i+1][0]}"))
        buttons.append(row)
...
async def game_start(callback: CallbackQuery, state: FSMContext):
    ...
    if game_id == "crash":
        ...
        await callback.message.edit_text(
            f"⚔️ *{game['name']}*\n\n"
            f"💰 Ставки: от {game['min_bet']} до {game['max_bet']} ⭐\n"
            f"🎴 Сравните карту с картой дилера!\n\n"
            f"Введите сумму ставки:",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Отмена", callback_data="games_menu")]])
        )
    elif game_id == "sicbo":
        await state.set_state(GameStates.playing_sicbo)
        await callback.message.edit_text(
            f"🎲 *{game['name']}*\n\n"
            f"💰 Ставки: от {game['min_bet']} до {game['max_bet']} ⭐\n"
            f"🎲 Три кости, угадайте исход!\n\n"
            f"Введите сумму ставки:",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Отмена", callback_data="games_menu")]])
        )
    elif game_id == "andar_bahar":
        await state.set_state(GameStates.playing_andar_bahar)
        await callback.message.edit_text(
            f"🌿 *{game['name']}*\n\n"
            f"💰 Ставки: от {game['min_bet']} до {game['max_bet']} ⭐\n"
            f"🎴 Угадайте, где выпадет карта!\n\n"
            f"Введите сумму ставки:",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Отмена", callback_data="games_menu")]])
        )
...
# Обработчики других игр (упрощены для краткости, но полные)
...
async def process_bet(message: Message, state: FSMContext):
    ...
    if game_id == "coinflip":
        ...
        await state.set_state(GameStates.playing_war)
        win, winnings, result_text = await play_war(message.from_user.id, bet)
        await message.answer(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        await state.set_state(GameStates.playing_sicbo)
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔴 Большое (11-17)", callback_data="sicbo_big"),
             InlineKeyboardButton(text="🔵 Малое (4-10)", callback_data="sicbo_small")],
            [InlineKeyboardButton(text="💎 Тройное (x30)", callback_data="sicbo_triple")]
        ])
        await message.answer("🎲 Выберите ставку:", reply_markup=keyboard)
        ...
        await state.set_state(GameStates.playing_andar_bahar)
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🌿 Андар", callback_data="andar_bahar_andar"),
             InlineKeyboardButton(text="🌾 Баар", callback_data="andar_bahar_bahar")]
        ])
        await message.answer("🌿 Выберите сторону:", reply_markup=keyboard)
...
@dp.callback_query(GameStates.playing_sicbo, F.data.startswith("sicbo_"))
async def sicbo_play(callback: CallbackQuery, state: FSMContext):
    choice = callback.data.replace("sicbo_", "")
    data = await state.get_data()
    bet = data["bet"]
    
    win, winnings, result_text = await play_sicbo(callback.from_user.id, bet, choice)
    
    await callback.message.edit_text(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎲 Ещё раз", callback_data="game_sicbo"),
         InlineKeyboardButton(text="🔙 В меню", callback_data="games_menu")]
    ]))
    await state.clear()
    await callback.answer()

@dp.callback_query(GameStates.playing_andar_bahar, F.data.startswith("andar_bahar_"))
async def andar_bahar_play(callback: CallbackQuery, state: FSMContext):
    choice = callback.data.replace("andar_bahar_", "")
    data = await state.get_data()
    bet = data["bet"]
    
    win, winnings, result_text = await play_andar_bahar(callback.from_user.id, bet, choice)
    
    await callback.message.edit_text(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌿 Ещё раз", callback_data="game_andar_bahar"),
         InlineKeyboardButton(text="🔙 В меню", callback_data="games_menu")]
    ]))
    await state.clear()
    await callback.answer()
...
async def casino_menu(callback: CallbackQuery):
    ...
    # Разбиваем казино игры на ряды по 2 кнопки
    casino_list = list(CASINO_GAMES.items())
    for i in range(0, len(casino_list), 2):
        row = []
        row.append(InlineKeyboardButton(text=casino_list[i][1]['name'], callback_data=f"casino_{casino_list[i][0]}"))
        if i + 1 < len(casino_list):
            row.append(InlineKeyboardButton(text=casino_list[i+1][1]['name'], callback_data=f"casino_{casino_list[i+1][0]}"))
        buttons.append(row)
...
async def casino_bet(message: Message, state: FSMContext):
    ...
    if game == "slots":
        ...
        win, winnings, result_text = await casino_war(message.from_user.id, bet)
        await message.answer(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        await state.set_state(CasinoStates.waiting_sicbo_bet)
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔴 Большое (11-17)", callback_data="sicbo_big"),
             InlineKeyboardButton(text="🔵 Малое (4-10)", callback_data="sicbo_small")],
            [InlineKeyboardButton(text="💎 Тройное (x30)", callback_data="sicbo_triple")]
        ...
        await state.set_state(CasinoStates.waiting_andar_bahar_choice)
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🌿 Андар", callback_data="andar_bahar_andar"),
             InlineKeyboardButton(text="🌾 Баар", callback_data="andar_bahar_bahar")]
        ])
...
@dp.callback_query(CasinoStates.waiting_sicbo_bet, F.data.startswith("sicbo_"))
async def casino_sicbo_choice(callback: CallbackQuery, state: FSMContext):
    choice = callback.data.replace("sicbo_", "")
    data = await state.get_data()
    bet = data["casino_bet"]
    
    win, winnings, result_text = await casino_sicbo(callback.from_user.id, bet, choice)
    
    await callback.message.edit_text(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎲 Ещё раз", callback_data="casino_sicbo"),
         InlineKeyboardButton(text="🔙 В меню", callback_data="casino_menu")]
    ]))
    await state.clear()
    await callback.answer()

@dp.callback_query(CasinoStates.waiting_andar_bahar_choice, F.data.startswith("andar_bahar_"))
async def casino_andar_bahar_choice(callback: CallbackQuery, state: FSMContext):
    choice = callback.data.replace("andar_bahar_", "")
    data = await state.get_data()
    bet = data["casino_bet"]
    
    win, winnings, result_text = await casino_andar_bahar(callback.from_user.id, bet, choice)
    
    await callback.message.edit_text(result_text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌿 Ещё раз", callback_data="casino_andar_bahar"),
         InlineKeyboardButton(text="🔙 В меню", callback_data="casino_menu")]
    ]))
    await state.clear()
    await callback.answer()
...
async def withdraw_menu(callback: CallbackQuery):
    ...
    cooldown_ok, cooldown_msg = await check_withdrawal_limit(callback.from_user.id)
    cooldown_status = "✅ Доступен" if cooldown_ok else f"⏳ {cooldown_msg}"
    
    text = (f"💰 *Вывод звезд*\n\n⭐ Баланс: {user['stars']}\n"
            f"💱 Курс: {settings.get('exchange_rate', 1)}:1 к звездам TG\n"
            f"📉 Минимум: {settings.get('min_withdraw', 500)} ⭐\n"
            f"⏱️ Статус: {cooldown_status}\n"
            f"📊 Выводов сегодня: {user['withdrawals_today']}/{settings.get('max_withdrawals_per_day', 3)}\n\n"
...
async def withdraw_stars(message: Message):
    ...
    cooldown_ok, cooldown_msg = await check_withdrawal_limit(message.from_user.id)
    if not cooldown_ok:
        await message.answer(f"❌ {cooldown_msg}")
        return
    
    await remove_stars(message.from_user.id, amount, f"Вывод звезд")
    await register_withdrawal(message.from_user.id)
...
async def show_stats(callback: CallbackQuery):
    ...
    text = (f"📊 *Ваша статистика*\n\n⭐ Баланс: {user['stars']}\n💰 Заработано: {user['total_earned']}\n"
            ...
            f"⏱️ Выводов сегодня: {user['withdrawals_today']}\n"
...
async def show_help(callback: CallbackQuery):
    settings = await load_json(SETTINGS_FILE, {})
    text = (f"❓ *Помощь*\n\n"
            f"🎮 *Игры (22):* Орёл/Решка, Кости, КНБ, Рулетка, Покер, Баккара, Блэкджек, Crash, Mines, Кено, Колесо Фортуны, Выше/Ниже, Хай-Ло, Plinko, Дуэль костей, Джекпот, Ракета, Монета, Счастливое число, Война, СИК БО, Андар Баар\n\n"
            ...
            f"⚔️ *War:* Карточная война с дилером!\n\n"
            f"🎲 *Sic Bo:* Три кости, угадайте исход!\n\n"
            f"🌿 *Andar Bahar:* Индийская карточная игра!\n\n"
            f"🎰 *Казино (12):* Слоты, Кости, Кено, Колесо Фортуны, Выше/Ниже, Plinko, Джекпот, Ракета, Угадай число, Война, СИК БО, Андар Баар\n\n"
...
# Админ панель (упрощена для краткости, но полная в предыдущих версиях)
