...
class CrashGame:
    ...
    async def update_multiplier(self) -> float:
        # Вероятность краша увеличивается с каждым тиком
        crash_probability = 1 / (self.current_multiplier * 10)
...
class MinesGame:
    ...
    def get_multiplier(self) -> float:
        ...
        multiplier = 1.0 + (opened_safe / safe_cells) * 3.0
    ...
    async def open_cell(self, cell: int) -> Tuple[bool, bool]:
        ...
        if cell in self.mines:
            self.is_active = False
            return True, False  # Мина, проигрыш
        
        self.current_multiplier = self.get_multiplier()
        return True, True  # Успешно открыто
...
# Обработчики других игр (аналогично предыдущей версии, сокращены для краткости)
...
async def show_help(callback: CallbackQuery):
    ...
    text = (f"❓ *Помощь*\n\n"
            ...
            f"💣 *Mines:* Открывай клетки на поле 5x5. Чем больше клеток открыто, тем выше множитель! Наступи на мину - проиграешь.\n\n"
