...
CHECK_ACCESS_FILE = os.path.join(DATA_DIR, "check_access.json")
...
async def init_data():
    ...
    check_access = await load_json(CHECK_ACCESS_FILE, {})
...
# Система чеков с кнопкой активации и платным доступом
async def buy_check_access(user_id: int) -> bool:
    """Покупка доступа к системе чеков"""
    settings = await load_json(SETTINGS_FILE, {})
    price = settings.get("check_access_price", 1000)
    
    user = await get_user(user_id)
    if user["stars"] >= price:
        await remove_stars(user_id, price, "Покупка доступа к чекам")
        await update_user(user_id, check_access=True)
        return True
    return False

async def has_check_access(user_id: int) -> bool:
    """Проверка доступа к системе чеков"""
    user = await get_user(user_id)
    return user.get("check_access", False)

async def create_check_with_button(user_id: int, amount: int) -> Tuple[bool, str, str, str]:
    """Создание чека с кнопкой активации"""
    settings = await load_json(SETTINGS_FILE, {})
    min_check = 100
    max_check = 100000
    ...
    if not await has_check_access(user_id):
        return False, "У вас нет доступа к системе чеков! Купите доступ в меню.", "", ""
    ...
    activation_link = f"https://t.me/{bot_username}?start=activate_check_{code}"
    
    return True, f"Чек создан!", code, activation_link

async def activate_check(user_id: int, code: str) -> Tuple[bool, str, int]:
...
async def play_dice(user_id: int, bet: int, choice: int) -> Tuple[bool, int, str]:
    ...
    win = (choice == result) and random.random() < 0.15
...
async def play_rps(user_id: int, bet: int, choice: str) -> Tuple[bool, int, str]:
    ...
    if win_condition and random.random() < 0.3:
...
async def play_roulette(user_id: int, bet: int, choice: str) -> Tuple[bool, int, str]:
    ...
    if choice == "red":
        ...
        win = result in red_numbers and result != 0 and random.random() < win_chance
        if win:
            winnings = int(bet * 1.9)
        ...
        win = result in black_numbers and random.random() < win_chance
        if win:
            winnings = int(bet * 1.9)
            await add_stars(user_id, winnings, f"Выигрыш в Рулетке")
            await update_tournament_points(user_id, winnings)
...
async def play_poker(user_id: int, bet: int) -> Tuple[bool, int, str]:
    ...
    win_chance = 0.35
    win = random.random() < win_chance and multiplier > 0
    
    if win:
        winnings = int(bet * multiplier * 0.8)
...
async def play_baccarat(user_id: int, bet: int, choice: str) -> Tuple[bool, int, str]:
    ...
    if choice == "player":
        win = player > banker and random.random() < win_chance
        if win:
            winnings = int(bet * 1.9)
        ...
        win = banker > player and random.random() < win_chance
        if win:
            winnings = int(bet * 1.9)
            await add_stars(user_id, winnings, f"Выигрыш в Баккаре")
            await update_tournament_points(user_id, winnings)
...
async def casino_slots(user_id: int, bet: int) -> Tuple[bool, int, str]:
    ...
    win_chance = 0.25
    win = random.random() < win_chance and win_multiplier > 0
...
async def casino_dice(user_id: int, bet: int, choice: int) -> Tuple[bool, int, str]:
    ...
    win = (choice == result) and random.random() < win_chance
...
# Система турниров с улучшенной механикой
...
async def update_tournament_points(user_id: int, points: int):
    ...
    if tournaments["active"] and settings.get("tournament_enabled", True):
        ...
        tournaments["active"]["total_bets"] += points
        await save_json(TOURNAMENTS_FILE, tournaments)
...
async def end_tournament():
    ...
    if tournaments["active"]:
        ...
        # Распределение призов: 1 место - 50%, 2 место - 30%, 3 место - 15%, остальные 5% на бонусы
        bonus_pool = int(prize_pool * 0.05)
        main_pool = prize_pool - bonus_pool
        
        for i, (user_id, points) in enumerate(sorted_participants[:3]):
            if i == 0:
                prize = int(main_pool * 0.5)
            elif i == 1:
                prize = int(main_pool * 0.3)
            else:
                prize = int(main_pool * 0.2)
        ...
        if bonus_pool > 0 and len(sorted_participants) > 3:
            bonus_per_user = bonus_pool // (len(sorted_participants) - 3)
            for user_id, points in sorted_participants[3:]:
                await add_stars(int(user_id), bonus_per_user, f"Бонус за участие в турнире")
                winners.append((user_id, bonus_per_user, points))
...
# Система заданий с проверкой бота в канале
async def add_task(admin_id: int, name: str, link: str, reward: int, required_bot: str = None) -> int:
    ...
    await log_admin_action(admin_id, "add_task", None, f"Name: {name}, Reward: {reward}, Bot: {required_bot}")
    return task_id
...
def get_main_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        ...
        [InlineKeyboardButton(text="📦 Система чеков", callback_data="check_system_menu"),
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if len(args) > 1:
        if args[1].startswith("activate_check_"):
            code = args[1].replace("activate_check_", "")
            success, msg, amount = await activate_check(message.from_user.id, code)
            await message.answer(f"{'✅' if success else '❌'} *{msg}*", parse_mode="Markdown")
            return
    
    # Обработка реферальной ссылки
    if len(args) > 1 and not args[1].startswith("activate_check_"):
        referrer_id = args[1]
    ...
    text = (f"✨ Добро пожаловать, {message.from_user.full_name}!\n\n"
            ...
            f"📦 Доступ к чекам: {'✅ Есть' if user.get('check_access', False) else '❌ Нет'}\n\n"
            f"🎮 6 увлекательных игр\n"
            f"🎰 Казино с высокими ставками\n"
...
# Система чеков с кнопкой
@dp.callback_query(F.data == "check_system_menu")
async def check_system_menu(callback: CallbackQuery, state: FSMContext):
    user = await get_user(callback.from_user.id)
    settings = await load_json(SETTINGS_FILE, {})
    access_price = settings.get("check_access_price", 1000)
    
    if user.get("check_access", False):
        text = (f"📦 *Система чеков*\n\n"
                ...
                f"• Максимум: 100000 ⭐\n"
                f"• Комиссия: 0%\n\n"
                f"📌 После создания вы получите ссылку с кнопкой для активации.\n"
                f"🔗 Активировать чек может любой пользователь по ссылке.")
        ...
        text = (f"📦 *Система чеков*\n\n"
                ...
                f"💎 Стоимость доступа: {access_price} ⭐\n\n"
                f"🔓 После покупки доступа вы сможете:\n"
                f"• Создавать чеки для друзей\n"
                f"• Получать звезды по чекам других пользователей\n"
                f"• Создавать чеки с кнопкой активации\n\n"
                f"💰 Создание чека: от 100 до 100000 ⭐")
        
        buttons = [
            [InlineKeyboardButton(text="🔓 Купить доступ", callback_data="buy_check_access")],
            [InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_main")]
        ]
...
@dp.callback_query(F.data == "buy_check_access")
async def buy_check_access_callback(callback: CallbackQuery):
    success = await buy_check_access(callback.from_user.id)
    settings = await load_json(SETTINGS_FILE, {})
    
    if success:
        await callback.answer("✅ Доступ к чекам куплен!", show_alert=True)
...
async def create_check_amount(callback: CallbackQuery, state: FSMContext):
    if not await has_check_access(callback.from_user.id):
    ...
    await callback.message.edit_text(
        ...
        "После создания вы получите ссылку с кнопкой для активации.",
        parse_mode="Markdown",
...
async def create_check_process(message: Message, state: FSMContext):
    try:
    ...
    success, msg, code, link = await create_check_with_button(message.from_user.id, amount)
    ...
    if success:
        await message.answer(
            ...
            f"{link}\n\n"
            f"📱 Или отправьте этот код другу: `{code}`",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📋 Скопировать ссылку", callback_data=f"copy_{link}")],
                [InlineKeyboardButton(text="🔙 В меню чеков", callback_data="check_system_menu")]
            ])
        )
    else:
        await message.answer(f"❌ {msg}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 В меню чеков", callback_data="check_system_menu")]
        ]))
...
async def my_checks(callback: CallbackQuery):
    ...
    if not created_checks:
        text = "📋 *Мои чеки*\n\nУ вас нет созданных чеков."
    else:
        text = "📋 *Мои чеки:*\n\n"
        for check in created_checks[-10:]:
            status = "✅ Активирован" if check.get("used") else "🟢 Активен"
            used_by = f" (активировал: {check['used_by']})" if check.get("used_by") else ""
            text += f"└ `{check['code']}` - {check['amount']} ⭐ - {status}{used_by}\n"
...
GAMES = {
    ...
    "rps": {"name": "✊ КНБ", "min_bet": 1, "max_bet": 500},
...
async def tournaments_menu(callback: CallbackQuery):
    ...
    if tournaments["active"]:
        ...
        text += f"\n💡 *Как участвовать:*\n• Делайте ставки в играх\n• Каждая ставка приносит очки\n• Чем выше ставка, тем больше очков"
    else:
        history = tournaments["history"]
        if history:
            last = history[-1]
            text = f"🏆 *Последний турнир*\n\n✨ {last['name']}\n💰 Приз: {last['prize_pool']} ⭐\n👥 Участников: {len(last['participants'])}\n🏆 Победитель: ID:{list(last['participants'].keys())[0] if last['participants'] else 'Нет'}\n\n📅 Завершен: {last['ended_at'][:10]}"
        else:
            text = "🏆 *Турниры*\n\nСейчас нет активных турниров. Следите за анонсами!"
...
async def show_stats(callback: CallbackQuery):
    ...
    text = (f"📊 *Ваша статистика*\n\n⭐ Баланс: {user['stars']}\n💰 Заработано: {user['total_earned']}\n"
            ...
            f"📦 Доступ к чекам: {'✅' if user.get('check_access', False) else '❌'}\n"
...
async def achievements_menu(callback: CallbackQuery):
    ...
    all_achievements = {
        "millionaire": "💰 Миллионер - Накопить 1,000,000 звезд",
        "high_roller": "🎲 Высокий игрок - Сыграть 1000 игр",
        "lucky": "🍀 Счастливчик - Выиграть 100 игр",
        "casino_king": "👑 Король казино - Выиграть 500 раз в казино",
        "referral_master": "👥 Мастер рефералов - Пригласить 100 друзей"
    }
    
    text = "🏆 *Ваши достижения*\n\n"
    for ach_id, ach_desc in all_achievements.items():
        if ach_id in achievements:
            text += f"✅ {ach_desc}\n"
        else:
            text += f"⬜ {ach_desc}\n"
    
    text += f"\n📊 Прогресс: {len(achievements)}/{len(all_achievements)} достижений"
...
async def show_help(callback: CallbackQuery):
    ...
    text = (f"❓ *Помощь*\n\n"
            ...
            f"• Рулетка | Покер | Баккара\n\n"
            f"🎰 *Казино:*\n"
            f"• Слоты | Кости\n\n"
            f"🏆 *Турниры:*\n"
            f"• Участвуйте в еженедельных турнирах\n"
            f"• Каждая ставка приносит очки\n"
            f"• Топ игроки получают призы\n\n"
            f"📋 *Задания:*\n"
            f"• Подписывайтесь на каналы\n"
            f"• Получайте звезды\n\n"
            f"👥 *Рефералы:*\n"
            f"• {settings.get('referral_reward', 10)} ⭐ за друга\n"
            f"• {settings.get('referral_percent', 10)}% от покупок друга\n\n"
            f"📦 *Система чеков:*\n"
            f"• Доступ покупается за {settings.get('check_access_price', 1000)} ⭐\n"
            f"• Создавайте чеки с кнопкой активации\n"
            f"• Активируйте чеки по ссылке\n\n"
            f"💰 *Вывод:*\n"
...
# =============== АДМИН ПАНЕЛЬ (сокращенная версия) ===============
...
async def admin_show_user(message: Message, state: FSMContext):
    ...
    try:
        ...
        text = (f"👤 *Пользователь {user_id}*\n\n⭐ Баланс: {user['stars']}\n💰 Заработано: {user['total_earned']}\n"
                ...
                f"✨ Достижений: {len(user['achievements'])}\n📦 Доступ к чекам: {'✅' if user.get('check_access', False) else '❌'}\n"
                f"📅 Регистрация: {user['created_at'][:10]}")
        buttons = [
...
async def admin_give_check_access(callback: CallbackQuery):
    ...
    await update_user(user_id, check_access=True)
    await callback.answer("✅ Доступ к чекам выдан!", show_alert=True)
    await admin_find_user(callback)
...
async def admin_mass_add_process(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ {count} пользователям добавлено {amount} ⭐")
...
async def admin_settings_menu(callback: CallbackQuery):
    ...
    text = (f"⚙️ *Настройки бота*\n\n"
            ...
            f"📦 Цена доступа к чекам: {settings.get('check_access_price', 1000)} ⭐\n\n"
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="🎲 Макс. ставка", callback_data="set_max_bet"),
         InlineKeyboardButton(text="📦 Цена доступа к чекам", callback_data="set_check_access_price")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_panel")]
    ]
    await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await callback.answer()
...
async def admin_setting_change(callback: CallbackQuery, state: FSMContext):
    ...
    names = {
        ...
        "check_access_price": "цену доступа к чекам"
    }
...
async def admin_stats(callback: CallbackQuery):
    ...
    active_checks = len(checks["checks"])
    open_tickets = len([t for t in support["tickets"] if t["status"] == "open"])
    users_with_check_access = len([u for u in users.values() if u.get("check_access", False)])
    ...
    text = (f"📊 *Общая статистика*\n\n"
            ...
            f"📦 Пользователей с доступом к чекам: {users_with_check_access}\n\n"
            f"📋 Заданий: {len(tasks)}\n"
