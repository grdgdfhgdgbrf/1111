...
# Система чеков (с доступом за плату)
        ...
        if task.get("required_bot"):
            try:
                ...
                if bot_member.status not in ["member", "administrator", "creator"]:
                    return False, f"❌ Бот {task['required_bot']} не найден в канале!"
            except:
                return False, f"❌ Бот {task['required_bot']} не найден в канале!"
...
async def buy_lottery_ticket(user_id: int, ticket_count: int) -> Tuple[bool, str]:
    ...
    await save_json(LOTTERY_FILE, lottery)
    return True, f"✅ Куплено {ticket_count} билетов! Всего билетов в лотерее: {len(lottery['active']['tickets'])}"
...
# Игры с низкими шансами
GAMES = {
    "coinflip": {"name": "🎲 Орёл или Решка", "min_bet": 1, "max_bet": 1000, "win_chance": 49, "multiplier": 1.95},
    "dice": {"name": "🎲 Кости", "min_bet": 1, "max_bet": 500, "win_chance": 16, "multiplier": 5.5},
    "rps": {"name": "✊ КНБ", "min_bet": 1, "max_bet": 500, "win_chance": 33, "multiplier": 2.7},
    "roulette": {"name": "🎡 Рулетка", "min_bet": 5, "max_bet": 2000, "win_chance": 48, "multiplier": 1.95},
    "poker": {"name": "🃏 Покер", "min_bet": 10, "max_bet": 5000, "win_chance": 40, "multiplier": 2.2},
    "baccarat": {"name": "🎴 Баккара", "min_bet": 10, "max_bet": 5000, "win_chance": 44, "multiplier": 1.95}
}
...
async def unlock_check_system(callback: CallbackQuery):
...
async def buy_lottery_ticket(callback: CallbackQuery, state: FSMContext):
...
async def show_help(callback: CallbackQuery):
    settings = await load_json(SETTINGS_FILE, {})
    text = (f"❓ *Помощь*\n\n"
            f"🎮 *Игры:* Орёл/Решка, Кости, КНБ, Рулетка, Покер, Баккара\n\n"
            f"🎰 *Казино:* Слоты, Кости\n\n"
            f"🏆 *Турниры:* Участвуйте, получайте очки, выигрывайте призы\n\n"
            f"📋 *Задания:* Подписывайтесь на каналы (проверка бота в канале)\n\n"
...
# Управление пользователями (упрощенно для краткости)
